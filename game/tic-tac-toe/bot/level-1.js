...
function calculateNear(grid, icon, row, col, offset) {
  if (grid[row][col] !== " ") {
    return -1;
  }
  let point = 0
...
function opponentWinCheck(grid, icon, offset) {
  const posWinGrid = genGridOf(3, 3);
  for (let row = 0; row < grid.length; row++) {
    for (let col = 0; col < grid.length; col++) {
      posWinGrid[row][col] = calculateNear(grid, icon, row, col, offset);
    }
  }
  return posWinGrid
}
...
function checkJustWin(grid, icon) {
  const scores = opponentWinCheck(grid, icon, 5)
...
function gameRound(grid, roundNo, player1, player1Icon, player2, player2Icon) {
  ...
  if (roundNo % 2 === 0) {
    delay();
    const offset = 10;
    const pointGrid = opponentWinCheck(grid, player1Icon, offset);
