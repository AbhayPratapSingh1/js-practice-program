...
const GRID_MARKER_TOP = ["1", "2", "3", "4", "5", "6", "7", "8"];
const GRID_MARKER_LEFT = ["A", "B", "C", "D", "E", "F", "G", "H"];
const GRID_INPUT_MESSAGE =
  "Enter the valid Input in format (left top) [eg-A1] : ";
...
const arrayofN = (n, placeHolder = " ") => {
  return " ".repeat(n).split("").map((each) => placeHolder);
};

const gridOfNM = (n, m, placeHolder = " ") => {
  return arrayofN(n).map((each) => arrayofN(m, placeHolder));
};

const deepCopy = (item) => {
  if (Array.isArray(item)) {
    return item.map((each) => deepCopy(each));
  }
  if (item === null) {
    return null;
  }
...
const overlapGrid = (base, overlaper, row = 0, col = 0) => {
  const newBase = deepCopy(base);
  for (let r = 0; r < overlaper.length; r++) {
    for (let c = 0; c < overlaper[r].length; c++) {
      if (newBase.length > r + row && newBase[r + row].length > c + col) {
        newBase[r + row][c + col] = overlaper[r][c];
      }
    }
  }
  return newBase;
...
const addGridMarker = (grid) => {
  const markingGrid = gridOfNM();
};
...
function getCordInput(message = GRID_INPUT_MESSAGE) {
  ...
  let isInvalid = input.length !== 2 || !isValidGridMarks(input[0], input[1]) ||
...
function checkWin(grid) {
  for (let row = 0; row < 3; row++) {
    if (
      ...
      grid[0][row] === grid[1][row] && grid[1][row] === grid[2][row] &&
      grid[0][row] !== " "
    ) {
      return grid[0][row];
    }
  }
  const digOneCheck = grid[0][0] === grid[1][1] && grid[0][0] === grid[2][2] &&
    grid[0][0] != " ";
  const digTwoCheck = grid[2][0] === grid[1][1] && grid[0][2] === grid[1][1] &&
    grid[2][0] != " ";
...
const cordinatePoint = function (grid, icon, row, col, offset) {
  ...
  points += checkRow(grid, icon, row, col, offset) +
    checkCol(grid, icon, row, col, offset);
  return points;
};
  ...
  for (let row = 0; row < 3; row++) {
    for (let col = 0; col < 3; col++) {
      if (grid[row][col] === " ") {
        ...
        const newAssumedPoints = genPointGrid(
          newAssumedGrid,
          icon,
          offset / 10,
          count,
...
function gameRound(
  ...
  if (winerIcon !== "") {
    const winnderMessage = `Congracts : ${
      winerIcon === player1Icon ? player1 : player2
    } ${winerIcon === player1Icon ? player1Icon : player2Icon}`;
  ...
  console.log(
    generateOutPut(grid, `${turnOfComputer ? player2 : player1} Turn!`),
  );
  let input = [];
...
function takeIconInput(message) {
  ...
  let isInvalid = typeof +input[1] !== "number" ||
    !(+input <= PLAYER_ICONS.length && +input > 0);
