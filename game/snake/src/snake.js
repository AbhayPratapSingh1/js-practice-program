...
const readAndClearFile = () => {
  const data = Deno.readTextFileSync("./src/inputBuffer.txt");
  Deno.writeTextFileSync("./src/inputBuffer.txt");
  return data;
};

const moveLeft = (snake) => {
  if (snake.dx === 0) {
    snake.dx = snake.dy === 1 ? 1 : -1;
    snake.dy = 0;
  } else {
    snake.dy = snake.dx === 1 ? -1 : 1;
    snake.dx = 0;
  }
};
