...
const frequencySet = (set, current) => {
  const element = set.find((each) => each[0] === current);
  if (element) {
    element[1] += 1;
    return set;
  }
...
const a1 = q1.reduce((count, color) => color === "blue" ? count + 1 : count, 0);
console.log("1.", a1);
...
const a2 = q2.flatMap((each) => each).reduce(unique_value, []);
console.log("2.", a2);
...
const a4 = q4.flatMap((each) => each).reduce(unique_value, []);
console.log("4.", a4);
...
const q5 = [[5, 3], [2], [4, 1]];
const a5 = q5.flatMap((each) => each).reduce(sumOfAll, 0);
console.log("5.", a5);
...
const a6 = q6.flatMap((each) => each).some((each) => each === "do");
console.log("6.", a6);
...
const a7 = q7.flatMap((each) => each).every((each) => each < 32);
console.log("7.", a7);
...
const a8 = q8.flatMap((each) => each).reduce(sumOfAll, 0);
console.log("8.", a8);
...
const a9 = q9.flatMap((each) => each).reduce(unique_value, []);
console.log("9.", a9);
...
const a16 = q16.flatMap((each) => each).reduce(unique_value, []);
console.log("16.", a16);
...
const a52 = q52.flatMap((each) => each).reduce((prev, each) =>
  prev + " " + each
);
console.log("52.", a52);
...
const a54 = q54.flatMap((each) => each).every((each) => each < 50);
console.log("54.", a54);

// ### 55. Unique Movie Genres
// Produce a list of unique movie genres mentioned by a club.
const q55 = [["movie1", "movie2", "movie3"], ["movie2", "movie3", "movie5"], [
  "movie6",
]];
const a55 = q55.flatMap((each) => each).reduce(unique_value, []);
console.log("55.", a55);

// ### 56. Pushup Total
// Compute the total number of pushups done in all sessions.
const q56 = [[12, 5, 4], [1, 42, 5]];
const a56 = q56.flatMap((each) => each).reduce(sumOfAll, 0);
console.log("56.", a56);
...
const a57 = q57.flatMap((each) => each).reduce(unique_value, []);
console.log("57.", a57);

// ### 58. Red Tile Count
// Count how many tiles in a mosaic were listed as “red”.
const q58 = [["blue", "green", "red"], ["orange", "red", "red"], [
  "red",
  "orange",
]];
const a58 = q58.flatMap((each) => each).reduce(
  (count, each) => each === "red" ? count + 1 : count,
  0,
);
console.log("58.", a58);
...
const a64 = q64.reduce((prev, current) => {
...
const a65 = q65.reduce(frequencySet, []);
console.log("65.");
console.table(a65);
...
const a66 = q66.flatMap((each) => each).some((each) => each === "fa");
console.log("66.", a66);

// ### 67. Parcel Weight Sum
// Sum all weights of parcels recorded in a courier office.
const q67 = [[12, 32, 12, 13], [23, 4, 23], [23, 54]];
const a67 = q67.flatMap((each) => each).reduce(sumOfAll, 0);
console.log("67.", a67);

// ### 68. Distinct Pizza Toppings
// List distinct toppings chosen by pizza customers.
const q68 = [["topping1", "topping2", "topping3"], [
  "topping2",
  "topping3",
  "topping4",
]];
const a68 = q68.flatMap((each) => each).reduce(unique_value, []);
console.log("68.", a68);
...
const a69 = q69.flatMap((each) => each).reduce((count, each) =>
  each === 2 ? count + 1 : count
);
console.log("69.", a69);

// ### 70. Combine Color Swatches
// Combine all color swatches from three design sets.
const q70 = [["color1", "color2", "color3"], ["color2", "color3", "color4"], [
  "color1",
  "color5",
]];
const a70 = q70.flatMap((each) => each).reduce(unique_value, []);
console.log("70.", a70);
...
const a71 = q71.flatMap((each) => each).some((each) => each >= 100);
console.log("71.", a71);

// ### 72. Completion Verification
// Verify whether every participant completed at least one task.
const q72 = [["pass", "fail", "fail", "pass"], ["fail", "fail", "pass"], [
  "pass",
  "pass",
  "pass",
]];
...
const a73 = q73.flatMap((each) => each).reduce(unique_value, []);
console.log("73.", a73);

// ### 74. Instrument Practice Sum
// Sum all hours spent practicing an instrument.
const q74 = [[12, 32, 54, 86], [13, 54, 8, 68], [90, 122, 32]];
const a74 = q74.flatMap((each) => each).reduce(sumOfAll, 0);
console.log("74.", a74);

// ### 75. Distinct Plant Types
// Find all distinct types of plants noted during fieldwork.
const q75 = [["plant1", "plant2", "plant3"], ["plant2", "plant3", "plant4"], [
  "plant1",
  "plant5",
]];
const a75 = q75.flatMap((each) => each).reduce(unique_value, []);
console.log("75.", a75);
...
const a81 = q81.flatMap((each) => each).reduce((prev, each) => {
  prev.unshift(each);
  return prev;
}, []);
console.log("81.", a81);
...
const q82 = [["train", "cran", "brain", "main", "rain"], ["jain", "rain"], [
  "rain",
  "raining",
]];
const a82 = q82.flatMap((each) => each).reduce((count, each) =>
  each === "rain" ? count + 1 : count
);
console.log("82.", a82);

// ### 83. Merge Poetry Fragments
// Merge all small poetry fragments into one list of lines.
const q83 = [["twinkle", "twinkle", "little", "star"], [
  "who",
  "stole",
  "my",
  "pickle",
...
const a89 = q89.flatMap((each) => each).reduce(sumOfAll, 0);
console.log("89.", a89);
...
const q91 = [["blue", "green", "white"], ["white", "red", "green"], [
  "orange",
  "white",
  "white",
]];
const a91 = q91.flatMap((each) => each).reduce((count, each) =>
  each === "white" ? count + 1 : count
);
console.log("91.", a91);

// ### 92. Combined Meeting Notes
// Combine all notes written during a meeting into one list.
const q92 = [["note_1", "note_2", "note_3"], ["note_4", "note_5", "note_6"], [
  "note_7",
  "note_8",
  "note_9",
]];
const a92 = q92.flatMap((each) => each);
console.log("92.", a92);

// ### 93. "Fragile" Item Check
// Check if any item in a shipment is marked “fragile”.
const q93 = [["fragile", "unknown", "unknown2"], [
  "fragile",
  "unknown",
  "unknown2",
], ["fragile", "unknown", "unknown2"]];
const a93 = q93.flatMap((each) => each).some((each) => each === "fragile");
console.log("93.", a93);
...
const a94 = q94.flatMap((each) => each).every((each) =>
...
const a95 = q95.flatMap((each) => each).reduce((prev, each) => {
  prev.unshift(each);
  return prev;
}, []);
console.log("95.", a95);
...
const a96 = q96.flatMap((each) => each).reduce(frequencySet, []);
console.log("96.");
console.table(a96);
...
const a97 = q97.flatMap((each) => each).reduce(
  (count, each) => each === "error" ? count + 1 : count,
  0,
);
console.log("97.", a97);
...
const a98 = q98.flatMap((each) => each).reduce(unique_value, []);
console.log("98.", a98);

// ### 99. Skipped Activity Check
// Check if any student skipped all activity sessions.
const student = ["student_1", "student_2", "student_3"];
const q99 = [["student_1", "student_2"], ["student_2", "student_1"], [
  "student_1",
  "student_2",
]];
const a99 = student.some((each) =>
  !q99.some((students) => students.includes(each))
);
console.log("99.", a99);

// ### 100. Distinct Hummed Songs
// Create a list of distinct songs hummed by children on a bus ride.
const q100 = [["song_1", "song_2", "song_3"], [
  "song_2",
  "song_3",
  "song_5",
  "song4",
]];
const a100 = q100.flatMap((each) => each).reduce(unique_value, []);
